...
heroImage:
  ...
  alt: 어두운 작업실에서 두 화면 사이에 빛줄기가 이어지고 한 손에 인쇄된 문서가 들린 사진 — 한쪽의 규범을 다른 쪽으로 옮겨 심는 시스템 프롬프트
...
시스템 프롬프트는 모델의 행동 양식을 바꿀 뿐, 모델의 지능(가중치)을 바꾸지 못한다. 이 프롬프트를 얹어도 Opus 4.8이 Fable 5가 되는 것은 아니다. 영상 제작자도 같은 점을 분명히 했다 — "Fable 5만큼의 성과가 나오지는 않지만, Opus 4.8의 능력을 최대치로 끌어내는 방법"이라고. 그 최대치가 생각보다 높다는 것이 이 방법의 실익이다. 추론 규율, 완결성, 정직성, 문장 품질처럼 프롬프트로 실제로 움직이는 영역에 집중해 설계한 이유다.
