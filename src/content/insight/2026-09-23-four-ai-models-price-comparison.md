...
새 모델이 나왔다고 강좌와 책의 전체 내용을 다시 쓸 필요는 없습니다. 대신 실습 안내에 모델 이름과 함께 **어느 화면에서 쓸 수 있는지, 어떤 요금제에서 가능한지, 긴 입력에 추가 요금이 붙는지**를 적어 두면 독자가 따라가기 쉽습니다. Sol·Luna는 일반 Chat 화면이 아니라 Work·Codex·API 경로라는 점, Luna Free·Go 이용은 데스크톱 앱에 한정된다는 점도 구분해야 합니다.[^4]

실습은 하나의 모델을 정답으로 제시하기보다 같은 자료를 두 후보에 주고 결과와 수정량을 비교하도록 만들 수 있습니다. 특히 한국어 과제는 공식 발표만으로 모델별 품질을 단정하지 말고, 수업에서 사용할 실제 문장과 평가 기준으로 확인하는 편이 낫습니다.

## 9. 아직 모르는 것

- 세 회사의 발표 자료는 국가별 롤아웃 차이를 다루지 않습니다. 지역별 제공 시점이 없다는 뜻이지, 지역 차이가 없다고 확인된 것은 아닙니다.
- Sol과 Luna가 일반 Chat 화면에 추가될 일정은 발표 자료에 없습니다.
- 네 모델의 한국어 품질을 같은 조건으로 비교한 결과는 없습니다.
- Grok 4.7의 파라미터 수는 공식 발표에 없어 이 글에서 다루지 않았습니다.
- Luna만 단독으로 다룬 X 사용 반응은 확인된 인용 후보에서 찾지 못했습니다.
- X의 지정 계정 20개 중 3개는 rate limit으로 확인하지 못했습니다.
- Grok의 코드 실행 도구는 확인한 공식 문서 경로에서 확인되지 않았습니다. 함수 호출·웹 검색·X 검색 지원과는 구분해야 합니다.
- 직접 비교 실험은 총 12회뿐이며 응답은 10회였습니다. Grok은 세 과제 중 한 번만 성공했습니다.
...
새 모델 네 개는 하나의 서열표보다 서로 다른 작업 후보로 읽는 편이 실용적입니다. 같은 자료와 기준으로 시험해, 쓸 수 있는 결과에 든 비용까지 비교해 보세요.
